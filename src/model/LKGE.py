...
class LKGE(BaseModel):
    ...
    def store_old_parameters(self):
        ...
        param_weight = self.get_new_weight() # 感觉这些weight都是用来计算MAE里面的那个reconstruct的
        self.args.snapshot += 1
        for name, param in self.named_parameters():
            ...
            # 这些weight我都不用改，因为只和样本中实体出现的次数有关
    ...
    def switch_snapshot(self):
        ...
        '''
        # 合并trainable和frozen数据
        # self.merge_parameters()
        '''store old parameters'''
        self.store_old_parameters() # 需要在这个之前完成frozen和trainable的合并
        '''expand embedding size for new entities and relations'''
        ent_embeddings, rel_embeddings = self.expand_embedding_size() # 这里返回的是随机初始化的结果
...
class TransE(LKGE):
    ...
    def MAE_loss(self):
        ...
        '''reconstruct'''
        ent_embeddings, rel_embeddings = self.embedding('Train') # 这个传入的stage没用
        try:
            old_entity_weight = self.old_weight_entity_embeddings
            old_relation_weight = self.old_weight_relation_embeddings
            old_x = self.old_data_entity_embeddings
    ...
    def loss(self, head, rel, tail=None, label=None):
        ...
        '''
        new_loss = self.new_loss(head, rel, tail, label)/head.size(0) # 这个应该就是TransE的loss
        loss = new_loss
...
class ConvLayer(nn.Module):
    ...
    def add_loop_edge(self, edge_index, edge_type, num_ent, num_rel):
        # 在知识图谱中为每个实体添加自环边（self-loop edge），从而使得在模型中每个实体在卷积运算时可以考虑自身的特征。
